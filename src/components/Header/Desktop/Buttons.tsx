            ...
            {isAuth ? (
                <>
                    ...
                    <Link href={`/${lang || 'en'}/chats`}>
                        <button className="button-inverse-Head">
                            {t('chats')}
                        </button>
                    </Link>
                    ...
                    <div className="relative" ref={menuRef}>
                        ...
                        {isMenuOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-50">
                                <div className="py-1">
                                    <Link href={`/${lang || 'en'}/profile`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition duration-200">
                                        {t('profile')}
                                    </Link>
                                    <Link href={`/${lang || 'en'}/catalog`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition duration-200">
                                        {t('catalog')}
                                    </Link>
                                    <Link href={`/${lang || 'en'}/blog`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition duration-200">
                                        {t('blog')}
                                    </Link>
                                    {isAdmin && (
                                        <Link href={`/${lang || 'en'}/admin`} className="block px-4 py-2 text-sm text-orange-600 hover:bg-orange-50 transition duration-200 font-medium">
                                            {t('admin')}
                                        </Link>
                                    )}
                                    <hr className="my-1" />
