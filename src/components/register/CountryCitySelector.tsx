...
export function CountryCitySelector({
    ...
    const getLocalizedName = (item: any): string =>
        item?.[`name_${lang}`] ?? item?.name_en ?? Object.values(item).find(v => typeof v === 'string') ?? '';
    ...
    return (
        <div className="w-full max-w-lg flex flex-col gap-2">
            ...
            >
                <option value="">-- {lang === 'uk' ? 'Оберіть країну' : 'Select country'} --</option>
                {countries.map(c => (
            ...
            <select
                ...
                disabled={!country}
            >
                <option value="">-- {lang === 'uk' ? 'Оберіть місто' : 'Select city'} --</option>
