...
export default function AdminPanel({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'users' | 'categories' | 'bids' | 'blogs'>('dashboard');
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_ENDPOINTS.adminUpdateUser(editForm.id)}`, {
        method: 'PUT',
        body: formData,
        credentials: 'include'
      });
      ...
      if (response.ok) {
        setEditingUser(null);
        setEditForm({ id: 0, name: '', email: '', user_role: '', nickname: '', city: '', profile_description: '' });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-6 px-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{stats?.chats_count || 0}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{stats?.categories_count || 0}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'users' && (
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b">
              <h3 className="text-lg font-medium">Користувачі ({users.length})</h3>
            </div>
        ...
        {activeTab === 'categories' && (
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b">
              <h3 className="text-lg font-medium">Категорії ({categories.length})</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Українська</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">English</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Polski</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Deutsch</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Français</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {categories.map((category) => (
                    <tr key={category.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">{category.name_uk}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{category.name_en}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{category.name_pl}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{category.name_de}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{category.name_fr}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Бюджет</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {bids.map((bid) => (
                    <tr key={bid.id}>
                      <td className="px-6 py-4 text-sm text-gray-900">{bid.id}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="max-w-xs truncate">{bid.title}</div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">{bid.budget || 'Не указан'}</td>
                      <td className="px-6 py-4">
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Управление блогами системы</p>
              </div>
              <button 
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">ID</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Заголовок (UK)</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Заголовок (EN)</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {blogs.map((blog) => (
                      <tr key={blog.id}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <div className="max-w-xs truncate">{blog.title_uk}</div>
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <div className="max-w-xs truncate">{blog.title_en || 'Не переведено'}</div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-500">
                          {new Date(blog.created_at).toLocaleDateString('uk-UA')}
                        </td>
                        <td className="px-6 py-4 text-sm font-medium space-x-2">
                          <button
        ...
        {editingUser && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              <h3 className="text-lg font-medium mb-4">Редактировать пользователя</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setEditForm({ ...editForm, profile_description: e.target.value })}
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  />
                </div>
              ...
              <div className="flex justify-end space-x-3 mt-6">
                <button
                  onClick={() => {
                    setEditingUser(null);
                    setEditForm({ name: '', email: '', user_role: '' });
                  }}
