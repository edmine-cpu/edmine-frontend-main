            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-2 bg-gradient-to-b from-gray-50 to-white">
              {messages.map((message, index) => {
                ...
                return (
                  <div
                    key={message.id}
                    className={`flex ${isOwn ? 'justify-end' : 'justify-start'} ${!showAvatar ? 'mt-1' : 'mt-4'}`}
                  >
                    {/* Avatar для чужих сообщений */}
                    {!isOwn && (
                      <div className={`flex-shrink-0 w-8 h-8 mr-3 ${showAvatar ? '' : 'invisible'}`}>
                    ...
                    >
                      ...
                      {message.file_path && (
                        <div className="mt-3">
                          ...
                          >
                            ...
                            {message.file_size && (
                              <span className="ml-2 text-xs opacity-75">({formatFileSize(message.file_size)})</span>
                            )}
                          </a>
                        </div>
                      )}
                      ...
                      <div className={`text-xs mt-2 ${
                        ...
                        {new Date(message.created_at).toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    </div>
                    
                    {/* Avatar для своих сообщений */}
                    {isOwn && (
                      <div className={`flex-shrink-0 w-8 h-8 ml-3 ${showAvatar ? '' : 'invisible'}`}>
