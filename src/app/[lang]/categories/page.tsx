...
interface BidItem { id: number; title_uk?: string; title_en?: string; title_pl?: string; title_fr?: string; title_de?: string; description_uk?: string; description_en?: string; city: string; created_at?: string; category?: string; under_category_id?: number; under_category?: { id: number }; budget?: string; currency?: string }
...
const T = {
  uk: { title: 'Категорії', orders: 'Замовлення', executors: 'Підрядники', services: 'Послуги', filters: 'Фільтри', category: 'Категорія', subcategory: 'Підкатегорія', country: 'Країна', city: 'Місто', sort: 'Сортування', byDate: 'по даті', byTitle: 'за назвою', byRelevance: 'за релевантністю', popular: 'популярні', recent: 'нові', select: '-- обрати --', details: 'Детальніше' },
  en: { title: 'Categories', orders: 'Orders', executors: 'Executors', services: 'Services', filters: 'Filters', category: 'Category', subcategory: 'Subcategory', country: 'Country', city: 'City', sort: 'Sort', byDate: 'by date', byTitle: 'by title', byRelevance: 'by relevance', popular: 'popular', recent: 'recent', select: '-- select --', details: 'Details' },
  pl: { title: 'Kategorie', orders: 'Zlecenia', executors: 'Wykonawcy', services: 'Usługi', filters: 'Filtry', category: 'Kategoria', subcategory: 'Podkategoria', country: 'Kraj', city: 'Miasto', sort: 'Sortowanie', byDate: 'po dacie', byTitle: 'według tytułu', byRelevance: 'według trafności', popular: 'popularne', recent: 'najnowsze', select: '-- wybierz --', details: 'Szczegóły' },
  fr: { title: 'Catégories', orders: 'Demandes', executors: 'Exécutants', services: 'Services', filters: 'Filtres', category: 'Catégorie', subcategory: 'Sous-catégorie', country: 'Pays', city: 'Ville', sort: 'Tri', byDate: 'par date', byTitle: 'par titre', byRelevance: 'par pertinence', popular: 'populaires', recent: 'récents', select: '-- sélectionnez --', details: 'Détails' },
  de: { title: 'Kategorien', orders: 'Aufträge', executors: 'Ausführende', services: 'Leistungen', filters: 'Filter', category: 'Kategorie', subcategory: 'Unterkategorie', country: 'Land', city: 'Stadt', sort: 'Sortierung', byDate: 'nach Datum', byTitle: 'nach Titel', byRelevance: 'nach Relevanz', popular: 'beliebt', recent: 'neu', select: '-- wählen --', details: 'Details' },
} as const;
...
export default function CategoriesPage({ params, searchParams }: { 
    ...
    return list.sort((a, b) => {
      ...
      switch (filters.sort) {
        ...
        case 'relevance':
          ...
          if (Math.abs(scoreDiff) < 86400000) { // если разница меньше дня
            return aTitle.localeCompare(bTitle, lang);
          }
          return scoreDiff;
        case 'popular':
  ...
  const filteredUsers = useMemo(() => {
    ...
    return list
      .filter(u => (filters.country ? String(u.country_id ?? u.country?.id) === filters.country : true))
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 flex items-start justify-center p-4">
        ...
        <h1 className="text-2xl font-semibold text-center text-red-600 mb-6">{t.title}</h1>
              ...
              <select value={filters.category} onChange={e => {
                ...
                if (categoryId) {
                  ...
                  if (category) {
                    ...
                    window.history.pushState(null, '', `/${lang}/catalog/${categoryName}`);
                  }
                } else {
        ...
        <div className="space-y-3">
          {filteredBids.map(b => {
            ...
            const catName = (b.category && b.category !== null && b.category !== '') ? 
              categories.find(c => String((c as any).id) === String(b.category))?.name || '' 
              : 'other';
            const subId = (b.under_category_id ?? b.under_category?.id) as any;
            const subList: any[] = Array.isArray(subcategories) ? (subcategories as any[]) : [];
            const subItem = subList.find(sc => String((sc as any).id) === String(subId));
            const subName = subItem ? getName(subItem) : '';
            
            // Find country and city names
            const countryName = (b as any).country_id ? 
              countries.find((c: any) => String(c.id) === String((b as any).country_id))?.name_en || '' : '';
            ...
            return (
              <div key={b.id} className="bg-white rounded-sm shadow p-4 border border-gray-200">
                ...
                <div className="text-xs text-gray-600 mb-2">{[catName === 'other' ? 'Інше' : catName, subName].filter(Boolean).join(' • ')}</div>
                ...
                <div className="flex items-center justify-between text-sm text-gray-600">
                  ...
                  <a href={`/${lang}/catalog/${(catName && catName !== 'other') ? catName.toLowerCase().replace(/\s+/g, '-') : 'other'}/${b.id}`} className="px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 transition-colors inline-block">
                    {t.details}
                  </a>
                </div>
              </div>
