...
export default function CatalogPage({ params, searchParams }: {
    ...
    const [activeTab, setActiveTab] = useState<'orders' | 'companies' | 'services'>('orders');
    ...
    const getName = (obj: any) => obj?.[`name_${lang}`] ?? obj?.name_en ?? obj?.name_uk ?? '';
    const getTitle = (obj: any) => obj?.[`title_${lang}`] ?? obj?.title_en ?? obj?.title_uk ?? '';
    const getDescription = (obj: any) => obj?.[`description_${lang}`] ?? obj?.description_en ?? obj?.description_uk ?? '';
    const getSlug = (obj: any) => obj?.[`slug_${lang}`] ?? obj?.slug_en ?? obj?.slug_uk ?? '';
            ...
            <div className="flex-1 flex items-start justify-center p-4">
                <div className="w-full max-w-6xl">
                    ...
                    <div className="flex justify-start gap-3 mb-6">
                        ...
                        <button onClick={() => setActiveTab('companies')} className={`px-4 py-2 rounded-md font-semibold ${activeTab==='companies' ? 'bg-red-600 text-white' : 'bg-white border text-gray-700'}`}>{t.companies}</button>
                            ...
                            <div>
                                ...
                                <select value={filters.category} onChange={e => {
                                    ...
                                    if (categoryId) {
                                        ...
                                        if (category) {
                                            const categoryName = getName(category).toLowerCase().replace(/\s+/g, '-');
                    ...
                    {activeTab === 'orders' && (
                        <div className="space-y-3">
                            {bids.map(bid => {
                                ...
                                return (
                                    <div key={bid.id} className="bg-white rounded-sm shadow p-4 border border-gray-200">
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="text-blue-700 font-semibold pr-4">{getTitle(bid) || '—'}</h3>
                                        ...
                                        <div className="text-xs text-gray-600 mb-2 space-y-1">
                                            {categoryNames && (
                                                <div>
                                                    <span className="font-medium">Категорія:</span> {categoryNames}
                                                </div>
                                            )}
                                            {subcategoryNames && (
                                                <div>
                                                    <span className="font-medium">Підкатегорія:</span> {subcategoryNames}
                                                </div>
                                            )}
                                        </div>
                    ...
                    {activeTab === 'companies' && (
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            {companies.map(company => {
                                ...
                                const companyUrl = `/${lang}/kompanii/${company.slug_name}`;

                                return (
                                    <a key={company.id} href={companyUrl} className="block text-left bg-white rounded-md shadow p-5 border border-gray-200 hover:shadow-md transition">
                                        <div className="mb-2">
                                            <h3 className="font-semibold text-blue-700 text-lg mb-1">{company.name}</h3>
                                            {location && (
                                        ...
                                        <div className="mt-auto text-xs text-blue-600 font-medium">{t.details} →</div>
                                    </a>
                                );
                            })}
                            {companies.length === 0 && (
