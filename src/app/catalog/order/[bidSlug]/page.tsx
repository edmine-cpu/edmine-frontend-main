...
export default function BidPage({
	...
	const getTitle = (b: Bid) =>
		b[`title_` as keyof Bid] || b.title_en || b.title_uk || 'No title'
	const getDescription = (b: Bid) =>
		b[`description_` as keyof Bid] ||
		b.description_en ||
		b.description_uk ||
		'No description'
		...
		return (
			ids
				.map(id => {
					...
					if (!category) return ''
					return (
						category[`name_` as keyof Category] ||
						category.name_en ||
						category.name_uk ||
						''
					)
		...
		return (
			ids
				.map(id => {
					...
					return (
						subcategory[`name_` as keyof Subcategory] ||
						subcategory.name_en ||
						subcategory.name_uk ||
						''
					)
	...
	const getCountryName = (id?: number) => {
		...
		if (!country) return ''
		return (
			country[`name_` as keyof Country] ||
			country.name_en ||
			country.name_uk ||
			''
		)
		...
		<div className='min-h-screen'>
			...
			<div className='max-w-4xl mx-auto px-4 py-8'>
				...
				<div className='bg-white rounded-lg shadow-md p-8'>
					...
					{/* Кнопка чата */}
					<ChatButton lang="en" authorId={bid.author} />
