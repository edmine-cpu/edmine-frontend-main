...
export default function BidPage({ params }: { params: Promise<{ categorySlug: string }> }) {
    const resolvedParams = React.use(params);
    const lang = "en" as Lang;
    const bidSlug = resolvedParams.categorySlug; // используем categorySlug как bidSlug
    const t = T[lang];
    ...
    const getTitle = (b: Bid) => b[`title_` as keyof Bid] || b.title_en || b.title_uk || 'No title';
    const getDescription = (b: Bid) => b[`description_` as keyof Bid] || b.description_en || b.description_uk || 'No description';
    ...
    const getCategoryNames = (ids?: number[]) => {
        ...
        return ids
            .map(id => {
                ...
                if (!category) return '';
                return category[`name_` as keyof Category] || category.name_en || category.name_uk || '';
            })
            .filter(Boolean)
            .join(', ') || '';
    };
    ...
    const getSubcategoryNames = (ids?: number[]) => {
        ...
        return ids
            .map(id => {
                ...
                return subcategory[`name_` as keyof Subcategory] || subcategory.name_en || subcategory.name_uk || '';
            })
            .filter(Boolean)
            .join(', ') || '';
    };
    ...
    const getCountryName = (id?: number) => {
        ...
        if (!country) return '';
        return country[`name_` as keyof Country] || country.name_en || country.name_uk || '';
    };
