...
export default function BidPage({ params }: {
    params: Promise<{ categorySlug: string; bidSlug: string }>
}) {
    ...
    const getCategoryNames = (ids?: number[]) => {
        ...
        return ids
            .map(id => {
                ...
                if (!cat) return '';
                const key = `name_` as keyof Category;
                return cat[key] || '';
            })
            .filter(Boolean)
            .join(', ');
    ...
    const getSubcategoryNames = (ids?: number[]) => {
        ...
        return ids
            .map(id => {
                ...
                if (!sub) return '';
                const key = `name_` as keyof Subcategory;
                return sub[key] || '';
            })
            .filter(Boolean)
            .join(', ');
    ...
    const getCountryName = (id?: number) => {
        ...
        if (!c) return '—';
        const key = `name_` as keyof Country;
        return c[key] || '—';
    };
