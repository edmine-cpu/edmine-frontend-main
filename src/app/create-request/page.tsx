...
export default function CreateBidPage() {
	...
	useEffect(() => {
		if (formState.country) {
			...
			setFormState(s => ({
				...s,
				city: filtered.some(c => c.id === Number(s.city)) ? s.city : '',
			}))
		} else {
			setFilteredCities([])
	...
	function handleLangFieldChange(
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		const hasDescription = Object.values(formState.description).some(
			v => v && v.trim()
		)
			...
			<div className='flex items-center justify-center px-4 pb-10'>
				<section className='max-w-150 bg-white rounded-md shadow-md p-8'>
					<div className='mb-4'>
						<LanguageSwitcher
							current={editLang}
							onChange={l => setEditLang(l)}
						/>
					</div>
					<h1 className='text-2xl font-semibold mb-6 text-red-600 text-center'>
						{t.title}
					</h1>
									...
									{block.category && (
										...
										>
											...
											{subcategories
												.filter(
													sc =>
														sc.category_id === block.category ||
														sc.full_category_id === block.category
												)
												.map(sc => (
